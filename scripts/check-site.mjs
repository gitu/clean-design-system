...
const REQUIRED_FILES = [
  { path: 'r/button.json', hint: 'the registry is copied by site/vite.config.ts' },
  { path: 'llms.txt', hint: 'generated by `pnpm registry`' },
  { path: 'favicon.svg', hint: 'lives in site/public/' },
  {
    path: 'storybook/index.html',
    // The likeliest cause by a mile, and one the bare filename does not suggest.
    hint: 'run `pnpm build-site` — `pnpm build-pages` empties site-dist/, and the Storybook goes in after it',
  },
]
...
for (const path of pages) {
  ...
  const found = await page.evaluate(() => ({
    ...
    // The way out. `Shell` turns the screen's own wordmark into a link, which
    // only works for a screen that renders its wordmark through `BrandMark` —
    // and a story that hand-rolls its masthead instead looks completely normal
    // while being a page you cannot leave.
    wayOut: document.querySelector('a.sb-masthead__brand--link')?.getAttribute('href'),
  }))

  const name = path === '' ? '(landing)' : path.replace(/\/$/, '')
  const isExample = path.startsWith('examples/') && path !== 'examples/'
  ...
  if (isExample && found.wayOut !== '../') {
    problems.push(`the wordmark does not link back to the index (href: ${found.wayOut ?? 'none'})`)
  }
...
for (const { path, hint } of REQUIRED_FILES) {
  try {
    await access(join(ROOT, path))
    console.log(`ok   ${path}`)
  } catch {
    failures.push({ name: path, problems: ['missing from the built site'] })
    console.log(`FAIL ${path}\n       missing from the built site — ${hint}`)
  }
}
